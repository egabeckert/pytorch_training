...
The thing that surprised me about conv layers here is that they work by essentially distorting the image in a predictable way. Each filter mangles the input differently, and those transformations surface abstract patterns — edges, textures, shapes — that the network can actually learn from. The image that comes out the other side of a conv layer looks nothing like what went in, but it's more useful to the model than the original was. Each layer transforms in its own way and hands something different downstream.
